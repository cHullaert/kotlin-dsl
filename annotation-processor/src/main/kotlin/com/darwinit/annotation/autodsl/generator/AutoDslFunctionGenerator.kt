...
class AutoDslFunctionGenerator(clazz: TypeElement,
    ...
    fun buildFunction(variableName: String): FunSpec {
        val lambda= LambdaTypeName.get(receiver = TypeVariableName(getPackageName() + "." + BUILDER_CLASS_PATTERN.format(clazz.simpleName.toString())),
            returnType = Unit::class.asClassName())

        val functionName = if(this.autoDsl.functionName.isNotBlank()) this.autoDsl.functionName else variableName

        return FunSpec.builder(functionName)
            .addParameter("block", lambda)
            .addStatement("%s = %s().apply(block).build()".format(variableName, getPackageName() + "." + BUILDER_CLASS_PATTERN.format(clazz.simpleName.toString())))
            .build()
    }
